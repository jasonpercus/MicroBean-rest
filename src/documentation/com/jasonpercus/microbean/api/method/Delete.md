...
## 🎯 Description
`@Delete` déclare qu'une méthode de controller traite une requête HTTP de type `DELETE`.
...
- Pourquoi ça existe: déclarer explicitement les handlers HTTP `DELETE` dans les controllers.
...
|-----------|-------------|------------|---------------------------------------------------------------------------------------|
| `value()` | `String`    | `public`   | Sous-chemin de la route `DELETE`, relatif au chemin `@Controller`. Par défaut : `""`. |
...
`@Delete` est conservée à l'exécution (`RetentionPolicy.RUNTIME`) et interprétée pendant la phase de scan des controllers pour créer les définitions de routes HTTP.
