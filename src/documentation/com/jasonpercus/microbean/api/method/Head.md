...
## 🎯 Description
`@Head` déclare qu'une méthode de controller traite une requête HTTP de type `HEAD`.
...
- Pourquoi ça existe: déclarer explicitement les handlers HTTP `HEAD` dans les controllers.
- Quel problème ça résout: permettre un point d'entrée sans corps de réponse, typiquement pour les métadonnées HTTP.
...
|-----------|-------------|------------|-------------------------------------------------------------------------------------|
| `value()` | `String`    | `public`   | Sous-chemin de la route `HEAD`, relatif au chemin `@Controller`. Par défaut : `""`. |
...
`@Head` est conservée à l'exécution (`RetentionPolicy.RUNTIME`) et interprétée pendant la phase de scan des controllers pour créer les définitions de routes HTTP.
