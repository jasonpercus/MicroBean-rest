...
## 🎯 Description
`@Options` déclare qu'une méthode de controller traite une requête HTTP de type `OPTIONS`.
...
- Pourquoi ça existe: déclarer explicitement les handlers HTTP `OPTIONS` dans les controllers.
- Quel problème ça résout: exposer les capacités HTTP d'une route (méthodes autorisées) de manière explicite.
...
|-----------|-------------|------------|----------------------------------------------------------------------------------------|
| `value()` | `String`    | `public`   | Sous-chemin de la route `OPTIONS`, relatif au chemin `@Controller`. Par défaut : `""`. |
...
`@Options` est conservée à l'exécution (`RetentionPolicy.RUNTIME`) et interprétée pendant la phase de scan des controllers pour créer les définitions de routes HTTP.
