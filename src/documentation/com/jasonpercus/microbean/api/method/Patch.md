# 📄 Patch [Type: Annotation]

## 🎯 Description
`@Patch` déclare qu'une méthode de controller traite une requête HTTP de type `PATCH`.
...
- Pourquoi ça existe: déclarer explicitement les handlers HTTP `PATCH` dans les controllers.
...
|-----------|-------------|------------|--------------------------------------------------------------------------------------|
| `value()` | `String`    | `public`   | Sous-chemin de la route `PATCH`, relatif au chemin `@Controller`. Par défaut : `""`. |
...
`@Patch` est conservée à l'exécution (`RetentionPolicy.RUNTIME`) et interprétée pendant la phase de scan des controllers pour créer les définitions de routes HTTP.
