...
## 🎯 Description
`@Put` déclare qu'une méthode de controller traite une requête HTTP de type `PUT`.
...
- Pourquoi ça existe: déclarer explicitement les handlers HTTP `PUT` dans les controllers.
...
|-----------|-------------|------------|------------------------------------------------------------------------------------|
| `value()` | `String`    | `public`   | Sous-chemin de la route `PUT`, relatif au chemin `@Controller`. Par défaut : `""`. |
...
`@Put` est conservée à l'exécution (`RetentionPolicy.RUNTIME`) et interprétée pendant la phase de scan des controllers pour créer les définitions de routes HTTP.
